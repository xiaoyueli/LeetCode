...
public class _364_NestedListWeightSum2 {
    ...
    private int getDepth(List<NestedInteger> nestedList) {
        ...
        int max = 0;
        for (NestedInteger sub: nestedList) {
            if (sub.isInteger()) continue;
            else {
                int level = getDepth(sub.getList());
    ...
    private int getSum(List<NestedInteger> ls, int level) {
        
        int sum = 0;
        for (NestedInteger nl: ls) {
            if (nl.isInteger()) sum += level * nl.getInteger();
            else sum += getSum(nl.getList(), level - 1);
        }
