 ...
 * 
 * 思路：
 * hashmap存到i的累计值, 如果=k. 更新max
 * 然后再check当前累计值-k后的值是否存在，若存在，则这个subarray的值=k, 更新max
 * 把当前累计值放入map
 *
 */
